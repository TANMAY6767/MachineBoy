...
const GamePage = () => {
  ...
  useEffect(() => {
    ...
    WebSocketClient.onClientIdReceived((clientId) => {
        // Prompt the user for their username and default to "Unknown" if nothing is entered
        ...
        // Update the frontend state for username
  ...
  const handleWebSocketMessage = (message) => {
    // Check if the message is a string before parsing it
    ...
    if (response.method === "rollDice") {
      setDiceResult(response.diceRoll);
    }
  
    if (response.method === "create") {
      setGameId(response.game.id);
      setCurrentTurn(response.creatorId);
    }
      ...
      <div id="divPlayers" style={playerContainerStyle}>
        {gameState?.clients.map((player) => (
          <div key={player.clientId} style={playerCardStyle}>
            ...
            {player.username || "Unknown"} ({player.color})
          </div>
        ))}
      </div>
